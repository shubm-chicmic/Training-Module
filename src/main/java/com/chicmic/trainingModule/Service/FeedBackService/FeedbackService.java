...
//        throw new ApiException(HttpStatus.BAD_REQUEST,"Task not assigned!!!");
    ...
    public com.chicmic.trainingModule.Dto.FeedbackResponseDto.FeedbackResponse saveFeedbackInDb(FeedbackRequestDto feedbackDto, String reviewerId, boolean role) {
        ...
        if (plan == null && !feedbackDto.getFeedbackType().equals(BEHAVIUOR_))
    ...
    public com.chicmic.trainingModule.Dto.FeedbackResponseDto.FeedbackResponse updateFeedback(FeedbackRequestDto feedbackRequestDto, String reviewerId) {
        ...
        Plan plan = mongoTemplate.findOne(query, Plan.class);
        if (plan == null && !feedbackV2.getType().equals(BEHAVIUOR_))
            throw new ApiException(HttpStatus.BAD_REQUEST, "Please enter valid planId!!");
...
//        Feedback_V2 feedbackV2 =  mongoTemplate.findOne(new Query(criteria), Feedback_V2.class);
        ...
        Plan plan = mongoTemplate.findOne(query, Plan.class);
        if (plan == null && !feedbackV2.getType().equals(BEHAVIUOR_))
            throw new ApiException(HttpStatus.BAD_REQUEST, "Invalid planId!!");
...
//               courseResponseList.get(index).reviewer().setOverallRating(roundOff_Rating(rating.getRating()/rating.getCount()));
    ...
    public PhaseResponse buildPhaseResponseForCourseOrTest(Feedback_V2 feedback_v2, Map<String, String> phaseDetails, Map<String, String> milestoneDetails) {
        ...
        if (feedback_v2.getType().equals(VIVA_)) {
            ...
            feedback_v2.getPhaseIds().forEach(st -> {
                phaseResponse.getSubTask().add(new UserIdAndNameDto(st, phaseDetails.get(st)));
            });
...
//            phaseResponse.setName(name.get(ratingCourse.getPhaseId()));
            ...
            feedback_v2.getMilestoneIds().forEach(st -> {
                phaseResponse.getSubTask().add(new UserIdAndNameDto(st, milestoneDetails.get(st)));
            });
            Collections.sort(phaseResponse.getSubTask());
...
//        ));
        ...
        if (type.equals(TEST_) || type.equals(VIVA_)) {
            ...
            if (milestoneObjectIds == null || milestoneObjectIds.isEmpty())
                throw new ApiException(HttpStatus.BAD_REQUEST, "No tasks available in this milestone!!");
